...
def compare_records(azara_record, rui_record):
    ...
    azara = azara_record[-1] # ES COMO 2B 
    rui_separado = rui_record[1]
    rui = rui_separado[0] + rui_separado[-1]
    if azara == rui:
        return True
    else:
        return False
...
def clean_up(combined_record_group):
    ...
    """
    lista_formateada = ""
    for pista in combined_record_group:
        lista_formateada = lista_formateada + f"('{pista[0]}'" + ", " + f"'{pista[2]}'" + ", " + f"{pista[3]}" + ", " + f"'{pista[4]}')" + "\n"
